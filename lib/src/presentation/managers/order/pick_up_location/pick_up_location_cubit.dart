...
class PickUpLocationCubit extends Cubit<PickUpLocationState> {
  ...
  double ecommerceLatitude = 30.3100485;
  double ecommerceLongitude = 31.7577825;
  double userLongitude = 0;
  double userLatitude = 0;
  List<LatLng> routesPoint = [];

  Future<void>doAction({required PickUpLocationAction pickedUpLocationAction})async{
    switch (pickedUpLocationAction) {
      case GetLocationUserDataAction(): return await _getLocationUserData();
    }
  }
  ...
  Future<void> _getLocationUserData() async {
    try {
      ...
      _isPermissionLocationUser = await locationManger.canGetLocationPermission();
      if (_isPermissionLocationUser) {
        ...
        emit(GetLocationUserFailuresState(errorMassage: "please User Open Location "));
      }
    } catch (e) {
  ...
  Future<void> _getRoutes() async {
    ...
    routesPoint = [];
    var result = await openRouteServiceApi.getRouteMaps(startLat: userLatitude, startLog: userLongitude, endLat: ecommerceLatitude, endLog: ecommerceLongitude);
    switch (result) {
